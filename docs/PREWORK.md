...
::: mac
下載下來是一個 `.dmg` 檔，打開它會跳出一個磁碟視窗，把裡面的 `Ollama.app` 拖進「應用程式」（Applications）資料夾，再打開它——選單列（畫面最上方）會出現一個小圖示，代表 Ollama 已經在背景執行。
:::

::: windows
下載下來是一個 `.exe` 安裝檔，雙擊執行，照畫面指示一路「下一步」安裝完成，它會自動把 `ollama` 加進系統路徑，並在工作列背景執行。**安裝完後請重新開一個新的終端機視窗**，路徑設定才會生效。
:::
